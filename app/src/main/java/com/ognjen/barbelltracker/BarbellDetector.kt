...
class BarbellDetector(
    ...
    fun detect(frame: Bitmap) {
        ...
        Log.d(TAG, "Inference time: $inferenceTime ms, box[0] x,y = ${bestBoxes?.get(0)?.x1}, ${bestBoxes?.get(0)?.y1}")

        if (bestBoxes == null) {
            detectionListener.onEmptyDetect()
            return
        }

        detectionListener.onDetect(bestBoxes, inferenceTime)
    }
